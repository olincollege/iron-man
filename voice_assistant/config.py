...
class Config:
    ...
    # Model selection
    TRANSCRIPTION_MODEL = (
        "deepgram"  # possible values: openai, groq, deepgram, fastwhisperapi
    )
    RESPONSE_MODEL = "groq"  # possible values: openai, groq, ollama
    TTS_MODEL = (
        "deepgram"  # possible values: openai, deepgram, elevenlabs, melotts, cartesia
    )
    ...
    def validate_config():
        ...
        """
        Config._validate_model(
            "TRANSCRIPTION_MODEL",
            ["openai", "groq", "deepgram", "fastwhisperapi", "local"],
        )
        Config._validate_model("RESPONSE_MODEL", ["openai", "groq", "ollama", "local"])
        Config._validate_model(
            "TTS_MODEL",
    ...
    def _validate_model(attribute, valid_options):
        ...
        if model not in valid_options:
            raise ValueError(f"Invalid {attribute}. Must be one of {valid_options}")

    @staticmethod
    def _validate_api_key(model_attr, model_value, api_key_attr):
        if getattr(Config, model_attr) == model_value and not getattr(
            Config, api_key_attr
        ):
